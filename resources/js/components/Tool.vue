<template>
    <div>
        <heading class="mb-6">Blog</heading>
        ...
        <card>
            ...
            <div class="px-6 py-8 text-80">
                <template v-if="installed">
                    <h4 class="mb-4">Blog is currently installed and active.</h4>
                ...
                <template v-else>
                    <p>
                        <span class="pr-4">Blog DB tables not found.</span>
...
<script>
    export default {
        ...
        methods: {
            installationCheck () {
                Nova.request().get("/nova-vendor/nova-blog-tool/check-migrations")
                    .then(response => this.installed = response.data.installed)
                    .catch(() => this.error = true)
            },

            install () {
                Nova.request().get("/nova-vendor/nova-blog-tool/migrate-tables")
            ...
            resetContent () {
                ...
                Nova.request().get("/nova-vendor/nova-blog-tool/reset-content")
                    .then(response => this.messages = response.data.messages)
                    .then(() => {
                        setTimeout(() => {
                            this.messages = [];
            ...
            deleteTables () {
                ...
                Nova.request().get("/nova-vendor/nova-blog-tool/uninstall")
                    .then(response => this.messages = response.data.messages)
                    .then(() => this.reloadPage())
                    .catch(error => this.error = true)
            },
